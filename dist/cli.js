...
\uD83D\uDCA1 Unexpected error. Please check AWS CLI logs`;
		...
		child.stdout?.on("data", (data) => {
			...
			if (
				...
				console.log(
					chalk2.green("\uD83C\uDF89 Port forwarding session started!"),
				);
			}
		});
		child.stderr?.on("data", (data) => {
			...
			if (output.includes("TargetNotConnected")) {
				...
				console.error(
					chalk2.yellow(
						"\uD83D\uDCA1 Please verify you have SSM-related IAM permissions",
					),
				);
	...
	while (retryCount <= maxRetries) {
		try {
			...
			if (retryCount <= maxRetries) {
				console.log(
					chalk4.yellow(
						`\uD83D\uDD04 Retry count: ${retryCount}/${maxRetries + 1}`,
					),
				);
				...
				console.log(
					chalk4.blue(`\uD83D\uDD04 Retrying...
`),
				);
			} else {
				console.log(
					chalk4.red("❌ Maximum retry count reached. Terminating process."),
				);
				console.log(
					chalk4.gray(
...
program
	...
	.action(async () => {
		try {
			console.log(
				chalk5.blue("\uD83D\uDE80 Starting AWS ECS RDS connection tool..."),
			);
			await connectToRDS();
